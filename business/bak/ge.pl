...
my ( $page_url, $all, $city, $region, $usca ) = ('', undef, undef, undef, undef);
my $valid = 'N';
...
foreach my $r (@{$aoh}) {
	...
	if ( defined $detail->[0] &&  $detail->[0] ) {
		...
		$sth = $dbh->do( q{ insert ignore into } . CONTACTS . qq{
			(google_keywords, meta_description, meta_keywords, title, url, phone, fax, email, zip, valid, summary, date, city, region) 
			values( '$keyword', $description, $keywords, $title, '$link', $phone, $fax, $email, $zip, '$valid', $summary, now(), '$city', '$region' )
		});
	}
	undef( @{$garef} );
	undef( @{$detail} );
